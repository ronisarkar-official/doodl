		...
		<>
			...
			<motion.div
				...
				<motion.div
					...
					className="relative p-6 sm:p-8 w-full max-w-md text-center"
					style={{
						background: '#202936',
					...
					:	<div className="flex flex-col items-center mb-4">
							...
							<div className="flex items-center gap-2 mt-3">
								...
								<span className="text-xl font-bold text-[#4fc3f7] capitalize drop-shadow-md">
									{currentWord}
								</span>
							</div>
						</div>
					...
					<div
						...
						style={{ background: '#2d3748', border: '3px solid #ffffff' }}>
						<div className="flex items-center gap-2 px-4 py-3 border-b-2 border-white/20 bg-white/5">
							{isGameEnd ?
								<Award className="w-5 h-5 text-[#ffb74d]" />
							:	<BarChart3 className="w-5 h-5 text-[#ffb74d]" />}
							...
							{sortedPlayers.map((player, index) => (
								<motion.div
									...
									<div className="flex items-center gap-3">
										<div
											className={`w-6 flex justify-center font-bold text-sm ${
										...
										<div className="flex items-center gap-2">
											...
											<div className="flex flex-col items-start">
												<span
													className={`text-sm font-semibold ${player.id === playerId ? 'text-primary' : 'text-foreground'}`}>
													{player.name}
												</span>
												{player.id === playerId && (
					...
					<div className="mt-8">
						{isGameEnd ?
							<div className="flex items-center gap-3">
								<motion.button
									...
									onClick={() => router.push('/')}
									className="flex-1 py-3 text-[16px] font-bold transition-all flex items-center justify-center gap-2"
									style={{
										background: '#cbd5e1',
										color: '#000000',
								...
								{isOwner && (
									<motion.button
										...
										whileTap={{ scale: 0.98, y: 4, boxShadow: 'none' }}
										onClick={resetGame}
										className="flex-2 py-3 text-[16px] font-bold transition-all flex items-center justify-center gap-2"
										style={{
											background: '#4fc3f7',
											color: '#000000',
											border: '3px solid #000000',
						...
						:	<div>
								...
								<div className="h-3 bg-[#2d3748] rounded-full overflow-hidden border-2 border-dashed border-[#a0aec0]">
									<motion.div
										initial={{ width: '0%' }}
										animate={{ width: '100%' }}
										transition={{ duration: 5, ease: 'linear' }}
										className="h-full bg-[#3bbdf5]"
									/>
								</div>
							</div>
						}
